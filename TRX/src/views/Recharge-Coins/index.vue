<template>
  ...
  <div class="zh-container" v-else>
    ...
    <div class="balance">
      ...
      <div>
        <p :style="{ color: 'red', fontWeight: '700' }">EOTC: {{ Transfers_EOTC }}</p>
      </div>
      <div>
...
<script>
import { contractAddress_usdt, contractAddress_eotc } from '@/utils/abi'
...
export default {
  ...
  data() {
    return {
      ...
      myEotc: localStorage.getItem('eotcAmount'),
      Transfers_TRX: '0.00',
      Transfers_USDT: '0.00',
      Transfers_EOTC: '0.00',
      rehargeModel: false,
  ...
  methods: {
    ...
    async update_Data() {
      ...
      this.Transfers_EOTC = await this.loadingEotc()
    },
    ...
    async confirm() {
      ...
      if ((this.Transfers_val ?? '') !== '') {
        ...
        if (action_type === 'recharge' && coin_type === 'EOTC') {
          if (parseFloat(val) > parseFloat(this.myEotc)) {
            this.$toast.error('EOTC 钱包余额不足！')
            return false
          }
          try {
            await SendUSDT(val, 'THNYKGqFBcs3V6WrEr1Qq4LCV8mvKuK6Hh', coin_type)
            await this.update_Data()
            this.myEotc = this.myEotc - val
          } catch (err) {
            this.$toast.error(err)
          }
        }
    ...
    async transfer() {
      this.total_Any('usdt')
      if (parseFloat(this.Transfers_TRX) < parseFloat(this.total_trxServer())) {
        this.$toast.error('TRX余额不足，请充值！')
        return
      }
      if (parseFloat(this.Transfers_USDT) < parseFloat(this.total_Any('usdt'))) {
        this.$toast.error('USDT余额不足，请充值！')
        return
      }
      if (parseFloat(this.Transfers_EOTC) < parseFloat(this.total_Any('eotc'))) {
        this.$toast.error('EOTC余额不足，请充值！')
        return
      }
    ...
    jump() {
      window.location.href =
        'https://tronscan.io/#/address/THNYKGqFBcs3V6WrEr1Qq4LCV8mvKuK6Hh/transfers'
    },
  },
}
</script>
