  ...
  <div class="content">
    <div class="top">
      ...
      <div class="currencyType">
        ...
        <div class="addPayBtn">
          <van-button @click="addpayFlgOpen(0, '', '')" class="btn bule" size="small">设置收付款账号</van-button>
        </div>
      </div>
    </div>
    <div v-if="receivingList.length > 0">
      <van-swipe-cell v-for="(receiving, index) of receivingList" :border="false" :key="index">
        <van-cell>
          <template #icon>
            <img
              class="pay-img"
    ...
    <!-- 选择当前交易货币类型弹窗 -->
    <van-popup v-model="showCurrencyType" round position="bottom" :style="{ height: '70%' }">
      <div class="searchType">
        <van-form @submit="onSearch" validate-trigger="onChange" ref="CurrencyTypeipt" class="ipt">
          <van-field
            name="moneyTtype"
