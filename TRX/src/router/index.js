...
const routes = [
	...
	},
	{
		...
		children: [
				...
				children: [
					{
						...
						component: () => import('@/components/ChooseWay/sell-currency-type')
					}
				]
			},
			{
			...
			},
			{
				...
				props: true,
				component: () => import('@/views/order-gather/order-undone/TemporaryCoinRefunds')
			},
			{
				path: 'RechargeCoins',
				name: 'RechargeCoins', // 隐藏 用户临时取币 转账 充值
				props: true,
...

	{
		...
		component: () => import('@/components/arbitration/forArbitrationDetails.vue')
	},
...
router.beforeEach((to, form, next) => {
	...
	if (to.name === 'order-Ticket') {
		...
		if (localStorage.getItem('otczy') * 1 < 5000 && Number(localStorage.getItem('giftNFT')) == 0) {
			Vue.$toast.warning({
				component: toastComponent,
				props: {
					title: '您质押的EOTC不足',
