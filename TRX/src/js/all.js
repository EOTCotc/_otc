...
exports.install = function (Vue, options) {
    //File 是图片上传完成后的返回值 state为0是添加水印 非0则不添加
    Vue.prototype.$reduce = function (File, state) { //全局函数1
            ...
            reader.onloadend = function () {
                ...
                if (this.result.length <= 300 * 1024) {
                    img.onload = function () {
                        ...
                        let fileZip = dataURLtoFile(base64, "id.jpg");
                        let asd = {
                            base64: base64, fileZip: fileZip
                        }
                        resolve(asd)
                    };
                } else {
...
function compress(img, show, state) {
    ...
    if (state == 0) {
        ...
        for (let i = 0; i < 15; i++) {
            for (let j = 0; j < 15; j++) {
                ctx.fillText('EOTC实名认证', i * (width + height) * 0.25, height * -0.3 + (height + width) * 0.1 * j, 300)
            }
        }
        ctx.restore()
    }
